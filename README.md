...
本番のD1は作成済み。`cloudflare/wrangler.toml` に接続先が書いてある。

| | |
| --- | --- |
| データベース名 | `selenia-aroma` |
| Database ID | `c06d174c-ab1f-4b82-9bb1-7b69a297980d`（秘密情報ではない） |
| ダッシュボード | https://dash.cloudflare.com/?to=/:account/workers/d1 |
...
### スキーマと初期データを流す
    ...
    npm run db:migrate          # 本番のD1へ
    npm run db:migrate:local    # 手元の仮想D1へ（確認用。本番に影響なし）

`cloudflare/d1/*.sql` を番号順に流す。**先に `:local` で試してから本番へ流すと安全。**
...
D1のコンソールは**入力欄が1行しかない**。複数行のSQLを貼ると改行が失われ、
先頭の `--` コメントが以降すべてをコメント扱いにするため1文も実行されない。

そのため通常は上記の `npm run db:migrate` を使う。どうしてもコンソールから
流す必要があるときのために、1行に潰して分割したものを
`cloudflare/d1/console/` に置いてある。
